...
def render_dual_video(
    ...
    *,
    ...
    figure.suptitle(
        "Two AWR1843BOOST sensors · parallel 3D comparison",
        color="#f4f7fb",
        fontsize=16,
        x=0.04,
        ha="left",
    ...
    figure.text(
        0.87, 0.925, "uncalibrated", color="#c9d5e2", fontsize=9
    )
    ...
    with writer.saving(figure, str(output_path), dpi=dpi):
        for index in range(len(hits_a)):
            ...
            _style(axis_3d, "Provisional 3D overlay")
            axis_3d.scatter(
            ...
            axis_3d.set(
                ...
                xlabel="x (m)", ylabel="depth (m)", zlabel="z (m)",
            )
            ...
            _style(axis_top, "Combined top view")
            axis_top.scatter(
