...
from lab.mmwave77_usb.background import robust_profile_statistics
...
class PerceptionSpec:
    ...
    person_reference_z_threshold: float = 6.0
    ...
    def validate(self) -> None:
        positive = (
            ...
            ("person_reference_z_threshold", self.person_reference_z_threshold),
        )
...
def _load_profile_context(
    ...
    person_reference_session: Path | None = None,
) -> dict[str, Any] | None:
    ...
    if person_reference_session is not None:
        reference_path = person_reference_session / "range_profiles.npz"
        if not reference_path.is_file():
            raise ValueError(
                "person reference has no range_profiles.npz: "
                f"{person_reference_session}"
            )
        with np.load(reference_path) as reference:
            reference_profiles = np.asarray(
                reference["profiles"], dtype=np.float32
            )
            reference_lengths = np.asarray(
                reference["profile_lengths"], dtype=np.int64
            )
        if reference_profiles.ndim != 2 or reference_profiles.shape[1] != len(median):
            raise ValueError(
                "person-reference and capture range-profile bins do not match"
            )
        complete = (
            (reference_lengths == len(median))
            & np.all(np.isfinite(reference_profiles), axis=1)
        )
        usable = reference_profiles[complete]
        if len(usable) < 40:
            raise ValueError(
                f"person reference has {len(usable)} complete profiles; "
                "requires at least 40"
            )
        statistics = robust_profile_statistics(usable, minimum_scale=1.0)
        result.update(
            {
                "person_reference_median": statistics["median"],
                "person_reference_scale": statistics["robust_scale"],
                "person_reference_p95": statistics["p95"],
                "person_reference_frames": int(len(usable)),
            }
        )
    return result
...
def _person_reference_compatibility(
    session: Path,
    person_reference_session: Path | None,
) -> tuple[bool, str]:
    if person_reference_session is None:
        return False, "no_person_reference"
    expected = ti_configuration_signature(person_reference_session)
    current = ti_configuration_signature(session)
    if not expected or not current:
        return False, "configuration_signature_unavailable"
    if str(expected) != str(current):
        raise ValueError(
            "participant and person-reference TI configurations are incompatible"
        )
    return True, "matching_ti_configuration_signature"
...
def _profile_z_for_window(
    ...
    *,
    ...
    if reference == "empty_room":
        median = context["median"]
        scale = context["scale"]
    elif reference == "person":
        if "person_reference_median" not in context:
            return None
        # Compare against the upper envelope observed without the test object,
        # not merely its median.  Normal body scintillation must therefore be
        # exceeded before a candidate can survive the matched-reference gate.
        median = context["person_reference_p95"]
        scale = context["person_reference_scale"]
    else:
        raise ValueError(f"unknown profile reference: {reference}")
    return ((current - median) / scale).astype(np.float32)
...
def _anomalies(
    ...
    *,
    ...
    person_profile_z: np.ndarray | None,
    calibration_compatible: bool,
    person_reference_compatible: bool,
    ...
    for label in sorted(set(int(value) for value in labels if value >= 0)):
        ...
        if (
            person_profile_z is not None
            and 0 <= profile_bin < len(person_profile_z)
        ):
            person_reference_z = float(person_profile_z[profile_bin])
            person_reference_component = _sigmoid(
                (person_reference_z - spec.person_reference_z_threshold) / 2.0
            )
            person_reference_available = person_reference_compatible
        else:
            person_reference_z = float("nan")
            person_reference_component = 0.0
            person_reference_available = False
        ...
        empty_room_score = float(
            sum(
                empty_room_components[name] * empty_room_weights[name]
                for name in empty_room_components
            )
        )
        if person_reference_compatible:
            components = {
                "person_reference_excess": person_reference_component,
                "snr_above_body_context": snr_component,
                "compactness": compactness_component,
                "motion_coupling": motion_coupling_component,
                "temporal_persistence": persistence_component,
            ...
            weights = {
                "person_reference_excess": 0.45,
                "snr_above_body_context": 0.15,
                "compactness": 0.15,
                "motion_coupling": 0.10,
                "temporal_persistence": 0.15,
            }
            score = float(
                sum(components[name] * weights[name] for name in components)
            )
            exceeds_person_reference = bool(
                person_reference_available
                and math.isfinite(person_reference_z)
                and person_reference_z >= spec.person_reference_z_threshold
            )
        else:
            components = empty_room_components
            weights = empty_room_weights
            score = empty_room_score
            exceeds_person_reference = True
        persistent = anomaly_track.hits >= spec.anomaly_min_persistence
        qualifies = bool(
            background_available
            and exceeds_person_reference
            and persistent
        ...
        results.append(
            {
                ...
                "person_reference_robust_z": (
                    person_reference_z if person_reference_available else None
                ),
                "person_reference_available": person_reference_available,
                "person_reference_excess": exceeds_person_reference,
                "persistence_windows": anomaly_track.hits,
                "components": components,
                "weights": weights,
                "empty_room_reflective_score": empty_room_score,
...
def run_perception(
    ...
    *,
    ...
    person_reference_session = (
        person_reference_session.expanduser().resolve()
        if person_reference_session is not None
        else None
    )
    frames_path = (
    ...
    person_reference_compatible, person_reference_reason = (
        _person_reference_compatibility(session, person_reference_session)
    )
    ...
    for window_index, (start, end) in enumerate(zip(frame_start, frame_end)):
        ...
        person_profile_z = _profile_z_for_window(
            profile_context,
            start_int,
            end_int,
            reference="person",
        ...
        if track is not None:
            ...
            anomalies, anomaly_tracks = _anomalies(
                ...
                track,
                profile_z=profile_z,
                person_profile_z=person_profile_z,
                calibration_compatible=calibration_compatible,
                person_reference_compatible=person_reference_compatible,
        ...
        elif track is None or selected is None:
            scene_state = "background"
            screening_state = (
        ...
        results.append(
            {
                ...
                "quality": {
                    ...
                    "person_reference_calibrated": (
                        person_reference_compatible
                    ),
                    "person_reference_reason": person_reference_reason,
                    "person_reference_residual_available": (
                        person_profile_z is not None
                    ),
    ...
    summary = {
        ...
        "person_reference_session": (
            str(person_reference_session)
            if person_reference_session is not None
            else None
        ),
        "background_calibrated": calibration_compatible,
        "calibration_compatibility_reason": calibration_compatibility_reason,
        "person_reference_calibrated": person_reference_compatible,
        "person_reference_reason": person_reference_reason,
        "person_reference_profile_frames": (
            int(profile_context.get("person_reference_frames", 0))
        ...
        "limitations": [
            ...
            "person-reference subtraction requires matched position and motion without the test object",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--person-reference-session",
        type=Path,
        help=(
            "matched capture of the same participant and motion without the "
            "test object; enables conservative body-reference subtraction"
        ),
    )
    ...
    parser.add_argument(
        "--person-reference-z-threshold",
        type=float,
        default=6.0,
        help="minimum robust range-profile excess above the matched person reference",
    )
    parser.add_argument(
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    spec = PerceptionSpec(
        ...
        person_reference_z_threshold=args.person_reference_z_threshold,
    ...
    try:
        output, summary_path, summary = run_perception(
            ...
            person_reference_session=args.person_reference_session,
