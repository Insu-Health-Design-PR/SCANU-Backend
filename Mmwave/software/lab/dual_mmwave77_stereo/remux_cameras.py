...
def remux_camera_to_host_time(camera: Path, camera_frames: Path, output: Path) -> None:
    """Create a camera MP4 whose playback clock is the receipt-time log.

    USB cameras often deliver a rate different from the rate requested from
    OpenCV.  The original MP4 is therefore only a frame container; seeking it
    by seconds may drift from the radar's ``host_monotonic_ns`` timeline.  The
    remuxed MP4 has one display duration per measured camera-frame interval.
    """
    timestamps = real_timestamps(camera_frames)
    with tempfile.TemporaryDirectory(prefix="scanu_camera_remux_") as tmp:
        frames = extract_frames(camera, Path(tmp), "camera")
        remux(frames, timestamps, output)
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument(
        "--camera-only",
        action="store_true",
        help="remux one USB-camera MP4 on its timestamp log; no diagnostic grid",
    )
    ap.add_argument("--camera", type=Path, help="source camera MP4 for --camera-only")
    ap.add_argument("--camera-frames", type=Path, help="timestamp JSONL for --camera-only")
    ap.add_argument("--perception-a", type=Path)
    ap.add_argument("--camera-a", type=Path)
    ap.add_argument("--camera-a-frames", type=Path)
    ap.add_argument("--trim-a-s", type=float)
    ap.add_argument("--perception-b", type=Path)
    ap.add_argument("--camera-b", type=Path)
    ap.add_argument("--camera-b-frames", type=Path)
    ap.add_argument("--trim-b-s", type=float)
    ...
    if args.camera_only:
        if args.camera is None or args.camera_frames is None:
            ap.error("--camera-only requires --camera and --camera-frames")
        remux_camera_to_host_time(args.camera, args.camera_frames, args.output)
        print(json.dumps({"output": str(args.output), "timebase": "host_monotonic_ns"}))
        return 0

    required = {
        "--perception-a": args.perception_a,
        "--camera-a": args.camera_a,
        "--camera-a-frames": args.camera_a_frames,
        "--trim-a-s": args.trim_a_s,
        "--perception-b": args.perception_b,
        "--camera-b": args.camera_b,
        "--camera-b-frames": args.camera_b_frames,
        "--trim-b-s": args.trim_b_s,
    }
    missing = [flag for flag, value in required.items() if value is None]
    if missing:
        ap.error("diagnostic grid requires " + ", ".join(missing))
