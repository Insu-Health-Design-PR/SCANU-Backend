#!/usr/bin/env python3
"""
Runtime entrypoint: Front Camera live + weapon_ai.infer_objects (multi-class).

Formerly used infer_thermal_objects (single-class AI Camera). Front Camera is now
the Multi-Camera classifier path (same as Back, local USB by default).

Run with cwd = New_Backend/Backend (see runtime.sensor_runner).

  python -m runtime.webcam_layer8_runner \\
    --checkpoint trained_models/gun_detection/gun_sohas_7class_phone_black_mix_v1.pt \\
    --webcam-device 0 \\
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Front Camera live infer (Multi-Camera / infer_objects).",
        allow_abbrev=False,
    )
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Path to multi-class gun YOLO .pt (e.g. gun_sohas_7class_phone_black_mix_v1.pt).",
    )
    p.add_argument(
        "--live-frame",
        type=str,
