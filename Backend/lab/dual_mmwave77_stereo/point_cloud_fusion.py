...
def _load_frames(frames_path: Path) -> tuple[list[int], list[int], list[np.ndarray]]:
    utc_ns: list[int] = []
    ...
    return utc_ns, frame_numbers, points


def _load_perception(perception_path: Path) -> tuple[list[int], list[int], list[float], list[bool], list[int], list[int]]:
    idx: list[int] = []
    fstart: list[int] = []
    tsec: list[float] = []
    present: list[bool] = []
    ...
    return idx, fstart, tsec, present, cluster_pts, input_pts


def load_session(session_dir: Path) -> SensorSession:
    frames_utc_ns, frame_numbers, points = _load_frames(session_dir / "frames.jsonl")
    idx, fstart, tsec, present, cluster_pts, input_pts = _load_perception(
        session_dir / "perception.jsonl"
    )
...
def cluster_points(points: np.ndarray, voxel_m: float = 0.18, min_points: int = 5) -> list[FusedCluster]:
    ...
    run_cent = np.empty((run_id[-1] + 1, 3), dtype=np.float32)
...
def run_fusion(
    ...
    b_by_window = {w: i for i, w in enumerate(session_b.window_index)}
    a_by_window = {w: i for i, w in enumerate(session_a.window_index)}

    spec = PerceptionSpec()
    fused_track: Any = None
    rows: list[dict[str, Any]] = []
    for i_a, w in enumerate(session_a.window_index):
        if w not in b_by_window:
            continue
        i_b = b_by_window[w]
        a_start = window_start_ns(session_a, session_a.window_frame_start[i_a])
        b_start = window_start_ns(session_b, session_b.window_frame_start[i_b])
        b_on_a = b_start - offset_ns
        if abs(b_on_a - a_start) > tol_ns:
            continue
        ...
        fused_track_present = (
            fused_track is not None
            and fused_selected is not None
        )
